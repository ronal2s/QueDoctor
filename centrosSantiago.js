...
class Centros extends Component
{
    state = 
    {
        liked: [{idcentro: 'a'}],
        likes: [126,133,12,89],
        city: "", centros:[], actualUser:"", userCode:"", filtro: "", modal: false, loading:true
    }
    ...
    fetchFiltro = async (value) => {
        const response = await fetch("https://serverquedoctor.herokuapp.com/centros/buscarEnCiudad?value=" + value+"&ciudad="+"Santiago De Los Caballeros");
        const body = response.json()
        if (response.status != 200) throw Error(body.message)
        return body;
    }
