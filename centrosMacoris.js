...
class Centros extends Component
{
    state = 
    {
        liked: [{idcentro: 'a'}],
        likes: [126,133,12,89],
        city: "", centros:[], actualUser:"", userCode:"", loading:true, filtro:"", modal: false
    }
    ...
    fetchFiltro = async (value) => {
        const response = await fetch("https://serverquedoctor.herokuapp.com/centros/buscarEnCiudad?value=" + value+"&ciudad="+"Macoris");
        const body = response.json()
        if (response.status != 200) throw Error(body.message)
        return body;
    }
